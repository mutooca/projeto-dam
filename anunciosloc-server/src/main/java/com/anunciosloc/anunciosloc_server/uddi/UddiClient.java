...
public class UddiClient {
    ...
    @SuppressWarnings("null")
    public String obterNomePorUrl(String url) {
        try {
            List<UddiRecord> infras = descobrirInfraestruturas();
            
            return infras.stream()
                .filter(r -> r.getServiceUrl().equals(url))
                .map(UddiRecord::getServiceName)
                .findFirst()
                .orElse(null);
                
        } catch (Exception e) {
            log.error("Erro ao obter nome por URL: {}", e.getMessage());
            return null;
        }
    }
}