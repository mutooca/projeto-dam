...
public class AnunciosSyncService extends Service {
            ...
            RetrofitClient.getInstance().getApi().anunciarLocalizacao(syncData).enqueue(new Callback<List<Anuncio>>() {
                ...
                public void onResponse(@NonNull Call<List<Anuncio>> call, @NonNull Response<List<Anuncio>> response) {
                    if (response.isSuccessful() && response.body() != null) {
                        for (Anuncio a : response.body()) showNewAnuncioNotification(a);
                    }
                }
    ...
    private void showNewAnuncioNotification(Anuncio anuncio) {
        ...
        PendingIntent pi = PendingIntent.getActivity(this, anuncio.getIdAnuncio().hashCode(), intent, flags);

        Notification n = new NotificationCompat.Builder(this, CHANNEL_ID)
                .setSmallIcon(R.mipmap.ic_launcher)
                .setContentTitle("Novo anúncio: " + anuncio.getTitulo())
                .setContentText(anuncio.getConteudo())
    ...
    private void createNotificationChannel() {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            NotificationChannel channel = new NotificationChannel(CHANNEL_ID, "Anúncios Locais", NotificationManager.IMPORTANCE_DEFAULT);
