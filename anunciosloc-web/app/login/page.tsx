    ...
    try {
      ...
      if (error.response) {
        if (error.response.status === 401) {
          errorMessage = 'Credenciais inválidas. Verifica o e-mail e palavra-passe.';
      ...
      <section className="flex flex-col md:w-1/2 w-full h-full items-center justify-center">
        <div className="flex flex-col max-w-md w-full mx-auto gap-4 p-4">
          ...
          <form onSubmit={handleSubmit(handleLogin)} className='space-y-2'>
            ...
            <Link href={'/recuperar_senha'} className='flex justify-end text-sm text-amber-500 hover:underline transition text-right'>
              Esqueceu a senha?
            </Link>
