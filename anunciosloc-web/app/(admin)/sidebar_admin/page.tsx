...
import { authService, AdminInfo } from '@/services/authServices';
...
export default function SidebarAdmin() {
    ...
    async function buscarAdminInfo() {
        try {
            setLoading(true);
            ...
            const info = await authService.getAdminInfo(email);
            setAdminInfo(info);
        } catch (error: any) {
    ...
    const userName = adminInfo?.nome || 'Carregando...';
    ...
    return (
        <div className="">
            <header className="fixed top-0 left-0 md:left-64 right-0 h-20 flex items-center justify-between bg-white border-b border-zinc-200 px-6 z-40">
                <button onClick={() => setMenuOpen(true)} className="md:hidden cursor-pointer">
                    <LuMenu size={24} />
                </button>
                <div className="hidden md:flex items-center gap-2">
                    <div className="bg-gray-200 border border-zinc-200 h-6 rounded-lg"></div>
                    <span className="text-zinc-600 sm:block">
                        AnunciosLoc / <span className="md:text-zinc-900 font-semibold">Painel do Gestor</span>
                    </span>
                </div>
                ...
                <div className="flex gap-4 items-center">
                    ...
                    <button 
                        ...
                        className="p-2 rounded-lg border border-zinc-100 flex items-center justify-center gap-2 text-zinc-600 cursor-pointer hover:bg-amber-500 hover:text-zinc-900 transition-colors"
                    >
                        <LuLogOut size={20} />
                    ...
                    <nav className="space-y-1">
                        {menuItems.map((item, index) => (
                            <Link
                                ...
                                onClick={() => setMenuOpen(false)}
                                href={item.href}
                                className='flex items-center gap-3 rounded-xl text-zinc-800 px-2 py-2 hover:bg-amber-200 transition-colors group'
                            >
                                <span className="text-zinc-800 group-hover:text-zinc-400 transition-colors">
                                    {item.icon}
                                </span>
